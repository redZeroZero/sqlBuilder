...
class ConditionGroupBuilderTest {
	...
	@Test
	void implicitSingletonGroupDropsParentheses() {
		var group = new ConditionGroupBuilder()
				.where(employee.get("FIRST_NAME")).eq("Alice")
				.build();

		assertThat(group.transpile()).isEqualTo("\"E\".\"FIRST_NAME\" = ?");
	}

	@Test
	void startOperatorIsPreservedWhenParenthesesAreRemoved() {
		var group = new ConditionGroupBuilder()
				.where(employee.get("FIRST_NAME")).eq("Alice")
				.build()
				.withStartOperator(Operator.AND);

		assertThat(group.transpile()).isEqualTo(" AND \"E\".\"FIRST_NAME\" = ?");
	}

	@Test
	void explicitSingletonGroupKeepsParentheses() {
		var group = QueryHelper.group()
				.where(employee.get("FIRST_NAME")).eq("Alice")
				.build();
